...
/// @dev Evidence-only checks for the undocumented V4 stack currently present
///      on Robinhood testnet. Passing these checks proves behavior and identity,
///      not official support or source provenance.
contract RobinhoodTestnetV4ForkTest is Test {
    ...
    function setUp() public {
        ...
        vm.skip(bytes(rpc).length == 0, "RH_TESTNET_RPC_URL is required");
    ...
    function test_dedicatedLockerLifecycleAgainstLiveTestnetManager() public {
        ...
        ArchV4PositionLocker locker = new ArchV4PositionLocker(
            0, payable(address(0xBEEF)), LockerV4PositionManager(address(manager)), address(this)
        );
