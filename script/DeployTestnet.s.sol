...
import {PoolKey} from "@archliquid/lockers/interfaces/IUniswapV4.sol";
...
contract DeployTestnet is Script {
    ...
    function run() external {
        ...
        // Fees default to the production schedule (lib/data.ts); override any via
        // env to stand up a low-fee instance for testing within a tight budget.
        ...
        // ── mock assets + V2/V3/V4 infrastructure ──
        ...
        MockDex v2Dex = new MockDex(stock);
        address v2Pair = v2Dex.createPair(address(stock), v2Dex.WETH());
        MockV4PoolManager v4PoolManager = new MockV4PoolManager();
        MockV4PositionManager v4PositionManager = new MockV4PositionManager(address(v4PoolManager));

        // ── core services ──
        ArchTreasury treasury = new ArchTreasury(gov);
        address payable t = payable(address(treasury));
        ArchLiquidityLocker v2Locker = new ArchLiquidityLocker(LOCKER_FEE, t, v2Dex, gov);
        ArchV3PositionLocker v3Locker = new ArchV3PositionLocker(LOCKER_FEE, t, gov);
        ArchV4PositionLocker v4Locker = new ArchV4PositionLocker(LOCKER_FEE, t, v4PositionManager, gov);
        ...
        ArchTokenFactory factory =
            new ArchTokenFactory(FACTORY_FEE, t, gov, infpm, iswap, iweth, FEE_TIER, v3Locker, registry);
        ...
        ArchLaunchpad launchpad = new ArchLaunchpad(
            LISTING_FEE, t, v3Locker, infpm, iswap, iweth, FEE_TIER, gov, registry, presaleDeployer, curveDeployer
        );
        ...
        v3Locker.setFeeExempt(address(factory), true);
        v3Locker.setFactory(address(launchpad), true);
        ...
        // though the fixture exposes a direct mint helper.
        stock.mint(v2Pair, 1_000e18);
        MockERC20(v2Dex.WETH()).mint(v2Pair, 10e18);
        MockV2Pair(v2Pair).sync();
        MockV2Pair(v2Pair).mint(gov, 100e18);
        (address currency0, address currency1) =
            address(stock) < address(usdc) ? (address(stock), address(usdc)) : (address(usdc), address(stock));
        v4PositionManager.mintPosition(
            gov,
        ...
        console2.log("V2 Factory/Router", address(v2Dex));
        console2.log("V2 WETH         ", v2Dex.WETH());
        console2.log("V2 Pair         ", v2Pair);
        console2.log("WETH            ", address(weth));
        console2.log("NFPM            ", address(nfpm));
        console2.log("SwapRouter      ", address(router));
        console2.log("V4 PoolManager  ", address(v4PoolManager));
        console2.log("V4 PositionMgr  ", address(v4PositionManager));
